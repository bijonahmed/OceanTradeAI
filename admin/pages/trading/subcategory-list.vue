<template>
    <title>Trading Sub Category List</title>
    <div>
                        ...
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                ...
                                <li class="breadcrumb-item active">
                                    <LazyNuxtLink to="/trading/add-sub-category">New</LazyNuxtLink>
                                </li>
                            </ol>
                        </div>
                    </div>
                            ...
                            <div class="row">
                                <div class="col-lg-8 col-md-8 col-sm-12 mb-2">
                                    <input type="text" v-model="searchQuery" class="form-control"
                                        placeholder="Search by trading category name" />
                                </div>
                                        ...
                                        <table class="table w-100 table-wrapper">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="text-center">Min Trade</th>
                                                    <th class="text-center">Trade Fee (%)</th>
                                            ...
                                            <tbody>
                                                <tr v-for="item in productdata" :key="item.id">
                                                    ...
                                                    <td class="text-left"><img v-if="item.images" :src="item.images" class="img img-thumbnail" alt="Image">{{ item.name }}</td> 
                                                    <td class="text-center">{{ item.min_trade }}</td> 
                                                    <td class="text-center">{{ item.trading_fee }}%</td> 
...
const selectedFilter = ref('0,1'); // Add a ref for the search query

const fetchData = async (page) => {
    try {
        loading.value = true;
        const response = await axios.get(`/trading/alltradingsubCategoryList`, {
            params: {
                page: page,
                pageSize: pageSize,
