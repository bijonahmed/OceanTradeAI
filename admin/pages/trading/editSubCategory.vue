<template>
    <title>Edit Trading Sub Category</title>
    <div>
              ...
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                  ...
                  <li class="breadcrumb-item active">
                    <LazyNuxtLink to="/trading/subcategory-list">Back to List</LazyNuxtLink>
                  </li>
                </ol>
              </div>
            </div>
                      ...
                      <div class="tab-content" id="custom-tabs-three-tabContent">
                        <div class="tab-pane fade active show" id="custom-tabs-three-home" role="tabpanel"
                          ...
                          <div class="row mb-3 required">
                            <label for="input-name-1" class="col-sm-2 col-form-label required-label">Select Category</label>
                            <div class="col-sm-10">
                                <select id="category" class="form-control" v-model="insertdata.category_id">
                                    <option v-for="option in productdata" :value="option.id" :key="option.id">{{ option.name }}
                                    </option>
                              </select>
                          ...
                          <div class="row mb-3 required">
                            <label for="input-name-1" class="col-sm-2 col-form-label required-label">Min Trade</label>
                            <div class="col-sm-10">
                              <input type="text"  placeholder="0.00" v-model="insertdata.min_trade" class="form-control" @keypress="isNumber($event)"/>
                              <span class="text-danger" v-if="errors.min_trade">{{ errors.min_trade[0] }}</span>
                            </div>
                          </div>


                          <div class="row mb-3 required">
                            <label for="input-name-1" class="col-sm-2 col-form-label required-label">Daily Trading Amount</label>
                            <div class="col-sm-10">
                              <input type="text"  placeholder="0.00" v-model="insertdata.trading_amunt" class="form-control" @keypress="isNumber($event)"/>
                              <span class="text-danger" v-if="errors.trading_amunt">{{ errors.trading_amunt[0] }}</span>
                            </div>
                          </div>


                          <div class="row mb-3 required">
                            <label for="input-name-1" class="col-sm-2 col-form-label required-label">Trading Fee (%)</label>
                            <div class="col-sm-10">
                              <input type="text"  placeholder="0.00" v-model="insertdata.trading_fee" class="form-control" @keypress="isNumber($event)"/>
                              <span class="text-danger" v-if="errors.trading_fee">{{ errors.trading_fee[0] }}</span>
                            </div>
                          </div>
                          ...
                          <div class="row mb-3">
                            <label for="input-meta-description-1" class="col-sm-2 col-form-label">Icon</label>
                            <div class="col-sm-10">
  ...
  const saveData = () => {
    ...
    formData.append('category_id', insertdata.category_id);
    formData.append('name', insertdata.name);
    formData.append('trading_fee', insertdata.trading_fee);
    formData.append('min_trade', insertdata.min_trade);
    formData.append('trading_amunt', insertdata.trading_amunt);
    formData.append('status', insertdata.status);
    formData.append('selected_graph', insertdata.selected_graph);
    console.log(formData);
  
    axios.post('/trading/saveSubCategory', formData, {
      headers: {
  ...
  const defaultLoadingrow = () => {
    ...
    axios.get(`/trading/tradingSubCategoryRow/${id}`).then(response => {
      insertdata.id = response.data.id;
      insertdata.name = response.data.name;
      insertdata.category_id = response.data.category_id;
      insertdata.min_trade = response.data.min_trade;
      insertdata.trading_amunt = response.data.trading_amunt;
      insertdata.selected_graph = response.data.selected_graph;
      insertdata.trading_fee = response.data.trading_fee;
