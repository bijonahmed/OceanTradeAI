<template>
    <title>Edit Trading Duration</title>
    <div>
              ...
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                  ...
                  <li class="breadcrumb-item active">
                    <LazyNuxtLink to="/trading/duration-list">Back to List</LazyNuxtLink>
                  </li>
                </ol>
              </div>
            </div>
                                    ...
                                    <div class="card-body">
                                        <div class="tab-content" id="custom-tabs-three-tabContent">
                                            <div class="tab-pane fade active show" id="custom-tabs-three-home"
                                                ...
                                                <div class="row mb-3 required">
                                                    <label for="input-name-1"
                                                        class="col-sm-2 col-form-label required-label">Percentage</label>
                                                    <div class="col-sm-10">
                                                        <input type="text" placeholder="%"
                                                            v-model="insertdata.percentage" class="form-control" @keypress="isNumber($event)" />
                                                ...
                                                <div class="row mb-3 required">
                                                    <label for="input-name-1"
                                                        class="col-sm-2 col-form-label required-label">Hours</label>
                                                    <div class="col-sm-10">
                                                        <input type="text" placeholder="Hours"
                                                            v-model="insertdata.hours" class="form-control" @keypress="isNumber($event)"/>
                                                        <!-- <input type="hidden" name="id" id="id" class="form-control" /> -->
                                                        <span class="text-danger" v-if="errors.hours">{{ errors.hours[0]
                                                            }}</span>
                                                    </div>
                                                </div>
...
const saveData = () => {
    ...
    formData.append('percentage', insertdata.percentage);
    formData.append('hours', insertdata.hours);
    formData.append('status', insertdata.status);
  
    console.log(formData);

    axios.post('/trading/saveDuration', formData, {
        headers: {
  ...
  const defaultLoadingrow = () => {
    ...
    axios.get(`/trading/tradingDurationRow/${id}`).then(response => {
      insertdata.id = response.data.id;
  
      insertdata.name = response.data.name;
      insertdata.status = response.data.status
  
      insertdata.hours = response.data.hours
      insertdata.percentage = response.data.percentage
